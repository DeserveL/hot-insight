...
- AI 辅助洞察：按突发公共事件、官方发布、辟谣核验等场景生成读者快读简报，包含一句话结论、事件梳理、关键事实、观察视角和参考来源。
...
- 首页：展示最新热点和重点洞察。
- 微博热搜：按标识浏览微博热点列表。
- 热点详情：展示微博实时材料、AI 洞察、关键事实和参考来源；存在实际争议或安全影响时展示风险提示。
...
| `HOT_TERM_EPISODE_GAP_HOURS` | 官方热搜词条离开榜单多久后再次出现记为新的爆发段 | `24` |
...
官方微博热搜的完整词条会单独保存为词条汇总和爆发段数据，不受 `TRACK_TAGS` 限制。系统会记录每个词条的累计出现轮次、复现次数、历史峰值和最好排名；词条离开榜单超过 `HOT_TERM_EPISODE_GAP_HOURS` 后再次出现，会记录为新的爆发段。该数据暂不对外展示，后续可用于年度热点、词条分析和词云图。
