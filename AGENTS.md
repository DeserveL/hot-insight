...
- 前端页面保持 Apple News / iOS 17 方向：`#F5F5F7` 高级灰背景、白色 Bento 卡片、超大圆角、极轻阴影、毛玻璃导航。
- 主题文字使用 `#1D1D1F`，辅助文字使用 `#86868B`；避免突兀高饱和红色，热度和标签使用低饱和莫兰迪色。
- 卡片、按钮、分段控制器和阅读模块必须复用项目内设计组件，新增页面不能回退到后台仪表盘或管理台风格。
- 列表筛选优先使用 iOS Segmented Control；内容列表优先使用 App Store 式卡片流；详情页使用上下段落式杂志阅读布局。
- 移动端优先：网格默认单列，卡片在移动端使用较小圆角和内边距，文字不得溢出或遮挡。
